...
peer lifecycle chaincode queryinstalled -O json | jq -r '.installed_chaincodes | .[] | select(.package_id|startswith("mycc_2.0:"))' > ccstatus.json
...
#   --name mycc --version 2.0 --init-required --sequence $SEQUENCE --tls \
